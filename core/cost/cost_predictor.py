...
# from DAC.DAC2025_Search_public.core.cost.input_converter import *
import numpy as np
...
class CostPredictor(nn.Module):
    ...
    def get_CIM_cost(self):
        flag = 0
        input = self.input
        memory = self.memory
        model = self.model
        layer_weight = model.weight.cpu().data.numpy()
        # terminal needs to be in the cost directory
        if len(model.weight.shape) > 2:
            k = model.weight.shape[-1]
            padding = model.padding
            stride = model.stride
            flag = 1

        if flag == 1:
            write_input_matrix = ic.stretch_input(
                input.cpu().data.numpy(), k, padding, stride
            )
            dimlist = np.array(
                [
                    input.shape[2],
            ...
            if(len(input.shape)==3):
                write_input_matrix=input.cpu().data.numpy().reshape((input.shape[0],-1))
            else:
                write_input_matrix = input.cpu().data.numpy()
            dimlist = np.array(
                [1, 1, write_input_matrix.shape[1], 1, 1, layer_weight.shape[0], 0, 1]
            ).reshape(1, -1)

        os.system(": > NSoutput.log")
        if memory == "reram":
            ic.write_matrix_weight(
                layer_weight,
                "../../Sims/DNN_NeuroSim_V1.4_reram/Inference_pytorch/layer_record_Final/weightConv0_.csv",
            )
            np.savetxt(
                "../../Sims/DNN_NeuroSim_V1.4_reram/Inference_pytorch/NeuroSIM/demolayer.csv",
                dimlist,
                delimiter=",",
                fmt="%s",
            )
            if flag:
                ic.write_matrix_activation_conv(
                    write_input_matrix,
                    None,
                    8,
                    "../../Sims/DNN_NeuroSim_V1.4_reram/Inference_pytorch/layer_record_Final/inputConv0_.csv",
                )
            else:
                ic.write_matrix_activation_fc(
                    write_input_matrix,
                    None,
                    8,
                    "../../Sims/DNN_NeuroSim_V1.4_reram/Inference_pytorch/layer_record_Final/inputConv0_.csv",
            ...
            os.system(
                "source ../../Sims/DNN_NeuroSim_V1.4_reram/Inference_pytorch/layer_record_Final/trace_command.sh >> NSoutput.log"
            )

        else:
            ic.write_matrix_weight(
                layer_weight,
                "../../Sims/DNN_NeuroSim_V1.4_sram/Inference_pytorch/layer_record_Final/weightConv0_.csv",
            )
            np.savetxt(
                "../../Sims/DNN_NeuroSim_V1.4_sram/Inference_pytorch/NeuroSIM/demolayer.csv",
                dimlist,
                delimiter=",",
                fmt="%s",
            )
            if flag:
                ic.write_matrix_activation_conv(
                    ...
                    None,
                    8,
                    "../../Sims/DNN_NeuroSim_V1.4_sram/Inference_pytorch/layer_record_Final/inputConv0_.csv",
                )
            else:
                ic.write_matrix_activation_fc(
                    write_input_matrix,
            ...
            os.system(
                "source ../../Sims/DNN_NeuroSim_V1.4_sram/Inference_pytorch/layer_record_Final/trace_command.sh >> NSoutput.log"
            )

        specs = {}
        with open("NSoutput.log", "r") as file:
            for line in file:
                a = line.strip().split()
                specs[a[0]] = float(a[1])
            print(specs)
